...
class Clipping:
    ...
    def __init__(self, viewportCoordenadas):
        ...
        # self.__pontosViewport = [
        #     EstruturaPonto(viewportCoordenadas[0], viewportCoordenadas[0]),
        #     EstruturaPonto(viewportCoordenadas[1], viewportCoordenadas[0]),
        #     EstruturaPonto(viewportCoordenadas[1], viewportCoordenadas[1]),
        #     EstruturaPonto(viewportCoordenadas[0], viewportCoordenadas[1])
        # ]
        self.__coordViewport = [EstruturaPonto(viewportCoordenadas[0], viewportCoordenadas[1]), EstruturaPonto(viewportCoordenadas[2], viewportCoordenadas[3])]
    ...
    def clip(self, objeto):
        ...
        # poligono
            ...
            self.__clippingSutherlandHodgeman(pontos)
            # for p in pontos:
                # novos.append(self.__clippingSutherlandHodgeman(p))
            # if len(novos) > 0:
                # objeto.setPontos(novos)
    ...
    def __clippingSutherlandHodgeman(self, pontos):
        novosPontos = []
        # Solução 1 - Clipar retas para formar novo polígono
        for i in range(0, len(pontos)):
            if i % 2 != 0:
                tmp = self.__clippingCohenSutherland(pontos[i-1], pontos[i])
                novosPontos.append(tmp[0])
                novosPontos.append(tmp[1])
        # Solução 2 - Mover pontos para a borda
        # if p.X() < self.__coordViewport[0].X():
        #     novoP.setX(self.__coordViewport[0].X())
        # if p.Y() < self.__coordViewport[0].Y():
        #     novoP.setY(self.__coordViewport[0].Y())
        # if p.X() > self.__coordViewport[1].X():
        #     novoP.setX(self.__coordViewport[1].X())
        # if p.Y() > self.__coordViewport[1].Y():
        #     novoP.setY(self.__coordViewport[1].Y())
        return novosPontos
