...
class Clipping:
    ...
    def clip(self, objeto):
        ...
        # dimensao 3
            ...
            segmentos = []
            for s in objeto.getSegmentosComZ():
                p1 = s.P1()
                p2 = s.P2()
                novos = self.__clippingCohenSutherland(p1, p2)
                if novos != []:
                    novoP1 = Ponto3D(novos[0].X(), novos[0].Y(), p1.Z())
                    novoP2 = Ponto3D(novos[1].X(), novos[1].Y(), p2.Z())
                    segmentos.append(SegmentoReta(novoP1, novoP2))
