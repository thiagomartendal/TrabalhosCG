...
class Normalizacao:
    ...
    def __init__(self, window, viewportCoordenadas):
        ...
        self.__projecaoOrtogonal = False
    ...
    def normalizar(self, objeto):
        if objeto.dimensao() == 2:
            ...
            if self.__projecaoOrtogonal:
                segmentosProj = self.__proj.segmentosProjParalelaOrtogonal(objeto)
            else:
                segmentosProjComZ, segmentosProj = self.__proj.segmentosProjPerspectiva(objeto)
                segmentosNormComZ = self.__normalizacao3D(objeto, segmentosProjComZ)
                objeto.setSegmentosNormalizadosComZ(segmentosNormComZ)
            #
    ...
    def __normalizacao3D(self, objeto, segmentosEntrada):
        ...
        for segmento in segmentosEntrada:
            ...
            matrizPonto2 = [segmento.P2().X(), segmento.P2().Y(), segmento.P1().Z()]
            ...
            p2 = Ponto3D(matrizRes2[0], matrizRes2[1], matrizRes1[2])
    ...
    def view(self, objeto):
        ...
        if objeto.dimensao() == 2:
            ...
            self.__view3DComZ(objeto, largura, altura)
    ...
    # Calcula a view 3D
    def __view3DComZ(self, objeto, largura, altura):
        segmentos = []
        for segmento in objeto.getSegmentosNormalizadosComZ():
            ponto1 = segmento.P1()
            ponto2 = segmento.P2()
            matrizPonto1 = [ponto1.X(), -ponto1.Y(), ponto1.Z()]
            matrizPonto2 = [ponto2.X(), -ponto2.Y(), ponto1.Z()]
            matrizEscal = [[largura/2,0,0],[0,altura/2,0],[0,0,1]]
            matrizResul1 = matmul(matrizPonto1, matrizEscal)
            matrizResul2 = matmul(matrizPonto2, matrizEscal)
            p1 = Ponto3D(matrizResul1[0]+largura/2, matrizResul1[1]+altura/2, matrizResul1[2])
            p2 = Ponto3D(matrizResul2[0]+largura/2, matrizResul2[1]+altura/2, matrizResul1[2])
            segmento = SegmentoReta(p1, p2)
            segmentos.append(segmento)
        objeto.setSegmentosComZ(segmentos)
