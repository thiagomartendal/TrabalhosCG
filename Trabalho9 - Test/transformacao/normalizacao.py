...
class Normalizacao:
    ...
    def normalizar(self, objeto):
        if objeto.dimensao() == 2:
            ...
            segAntigos, segmentosProj = self.__proj.segmentosProjPerspectiva(objeto)
            segmentosNorm = self.__normalizacao3D(objeto, segmentosProj)
            segmentosNorm2 = self.__normalizacao3D(objeto, segmentosProj)
            objeto.setSegmentosNormalizados(segmentosNorm)
            objeto.setSegmentosNormalizados2(segmentosNorm2)
    ...
    def view(self, objeto):
        ...
        if objeto.dimensao() == 2:
            ...
            self.__view3D2(objeto, largura, altura)
    ...
    # Calcula a view 3D
    def __view3D2(self, objeto, largura, altura):
        segmentos = []
        for segmento in objeto.getSegmentosNormalizados2():
            ponto1 = segmento.P1()
            ponto2 = segmento.P2()
            ...
            segmento = SegmentoReta(p1, p2)
            segmentos.append(segmento)
        objeto.setSegmentos2(segmentos)
